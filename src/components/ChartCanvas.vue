...
function bindClickHandlers() {
  ...
  el.value.on('plotly_click', (ev) => {
    ...
    if (props.spec.type === 'waterfall') {
      emit('clickFilter', { key: props.spec.x, value: pt.x });
      return;
    }
    if (props.spec.type === 'funnel') {
      emit('clickFilter', { key: props.spec.x, value: pt.y ?? pt.label });
      return;
    }
    if (props.spec.type === 'pie' || props.spec.type === 'donut') {
      emit('clickFilter', { key: props.spec.x, value: pt.label });
      return;
    }
    if (props.spec.type === 'area') {
      emit('clickFilter', { key: props.spec.x, value: pt.x });
      return;
    }
...
function plotFromSpec() {
  ...
  function aggregateByX() {
    ...
    for (const r of rows) {
      const key = String(val(r, x) ?? '');
      const n = Number(r?.[y]);
      counts.set(key, (counts.get(key) || 0) + 1);
      if (Number.isFinite(n)) sums.set(key, (sums.get(key) || 0) + n);
      else if (!sums.has(key)) sums.set(key, 0);
    }
    const keys = Array.from(counts.keys());
    const sorted = sortCats(keys, options?.sortCategory || 'none', Object.fromEntries(sums));
    const values = sorted.map((k) => {
      if (!y || agg === 'count') return counts.get(k) || 0;
      if (agg === 'sum') return sums.get(k) || 0;
      if (agg === 'mean') return (sums.get(k) || 0) / Math.max(counts.get(k) || 1, 1);
  ...
  if (type === 'bar') {
    ...
    Object.keys(groups).forEach((k, i) => {
      ...
      traces.push({
        ...
        stackgroup: isArea && options?.stackedArea !== false ? 'one' : undefined,
    ...
    const groups = groupBy(hue);
    const sizeCol = size || options?.sizeCol || '';
    data = Object.keys(groups).map((k, i) => {
      const arr = groups[k];
      const sizeVals = sizeCol
        ? arr.map((r) => Math.max(0, Number(r?.[sizeCol]) || 0))
        : arr.map(() => 12);
      ...
      return {
        ...
        marker: {
          ...
          sizemin: 4,
          opacity: 0.75,
          color: pal ? pal[i % pal.length] : undefined,
        },
      };
    });
    ...
    const { labels, values } = aggregateByX();
    data = [
      {
        type: 'pie',
        labels,
    ...
    const { labels, values } = aggregateByX();
    data = [{ type: 'funnel', y: labels, x: values }];
    layout.xaxis.title = options?.yLabel || y || options?.agg || 'value';
    layout.yaxis.title = options?.xLabel || x || '';
  } else if (type === 'waterfall') {
    const { labels, values } = aggregateByX();
    data = [{ type: 'waterfall', x: labels, y: values, measure: labels.map(() => 'relative') }];
