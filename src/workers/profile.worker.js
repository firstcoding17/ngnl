...
self.onmessage = (e) => {
  ...
  if (!rows || !rows.length) {
    return self.postMessage({
      ...
      data: {
        ...
        topCorr: [], topAnova: [], warnings: ['No data available.']
      }
    });
  }
  if (rows.length < 10) {
    warnings.push('Sample size is too small (<10 rows). Profile metrics may be unreliable.');
  }
  ...
  if (!cols.length) {
    return self.postMessage({
      ...
      data: {
        ...
        topCorr: [], topAnova: [], warnings: [...warnings, 'No columns found.']
      }
    });
  }
  ...
  // 2) Duplicate rows (sample-based)
  const dupCount = countDuplicates(S);

  // 3) Top-10 correlations (numeric only)
  ...
  if (!corrPairs.length) warnings.push('Not enough valid pairs for correlation (minimum 3).');
  corrPairs.sort((a, b) => Math.abs(b.r) - Math.abs(a.r));
  const topCorr = corrPairs.slice(0, 10);

  // 4) ANOVA Top-10 (categorical -> numeric)
  const anovaRaw = [];
  ...
  if (!anovaRaw.length) warnings.push('No category-to-numeric combinations meet ANOVA requirements.');
