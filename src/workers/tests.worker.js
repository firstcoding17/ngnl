...
function doTTest({ rows, mode, colA, colB, oneCol, mu=0, alternative='two-sided' }){
  ...
  if (mode==='one-sample'){
    ...
    if (a.length<2) return post({ ok:true, data:{ type:'one-sample', error:'Insufficient sample size (>=2 required).' }});
  ...
  if (mode==='paired'){
    ...
    if(n<2) return post({ ok:true, data:{ type:'paired', error:'Insufficient paired sample size (>=2 required).' }});
    ...
    if (na<2||nb<2) return post({ ok:true, data:{ type:'independent', error:'Each group must have at least 2 samples.' }});
...
function doNormality({ rows, column }){
  ...
  if (x.length<8) return post({ ok:true, data:{ column, test:'Jarque-Bera', error:'Insufficient sample size (>=8 recommended).' }});
...
function doLevene({ rows, column, group }){
  ...
  if (G.length<2) return post({ ok:true, data:{ test:'Levene(BF)', error:'Insufficient groups (>=2 groups, each >=2 samples).' }});
